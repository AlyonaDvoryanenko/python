...
class Clothes:    # родительский класс
    def __init__(self, size, height):
        self.size = size
        self.height = height

    def get_square_c(self):  # расчет площади ткани на пальто
        return self.size / 6.5 + 0.5

    def get_square_s(self):  # расчет площади ткани на костюм
        return self.height * 2 + 0.3

    @property  # работаем с методом класса Clothes
    def get_sq_full(self):
        return str(f'Общая площадь ткани \n'
                   f' {(self.size / 6.5 + 0.5) + (self.height * 2 + 0.3)}')  # складываем площадь на пальто и костюм
